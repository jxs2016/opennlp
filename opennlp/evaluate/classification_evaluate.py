...
class ClassificationEvaluator(object):
    ...
    def __init__(self, eval_dir, greedy=True):
    ...
    def _calculate_prf(right_count, predict_count, standard_count):
        ...
        if standard_count > 0:
            accuracy = right_count / standard_count
        if predict_count > 0:
    ...
    @staticmethod
    def _calculate_accuracy(right_count, standard_count):
        """Calculate precision, recall, fscore
        Args:
            standard_count: Standard count
            predict_count: Predict count
    ...
    def calculate_level_performance(
        ...
        recall_dict = dict()
        fscore_dict = dict()
        accuracy_dict[self.MACRO_AVERAGE] = 0
        ...
        for _, label_name in id_to_label_map.items():
            if label_name in other_label:
                continue
            accuracy_dict[label_name], precision_dict[label_name], recall_dict[label_name], \
            ...
            accuracy_dict[self.MACRO_AVERAGE] += accuracy_dict[label_name]
        ...
        accuracy_dict[self.MACRO_AVERAGE] = \
            accuracy_dict[self.MACRO_AVERAGE] / num_label_eval
        ...
        (accuracy_dict[self.MICRO_AVERAGE],
         precision_dict[self.MICRO_AVERAGE],
         recall_dict[self.MICRO_AVERAGE],
         fscore_dict[self.MICRO_AVERAGE]) = \
            self._calculate_prf(right_total, predict_total, standard_total)
        return accuracy_dict, precision_dict, recall_dict, fscore_dict

    def evaluate(self, predicts, standard_label_names=None,
                 standard_label_ids=None, label_map=None, threshold=0, top_k=20,
                 is_prob=True, is_flat=True, is_multi=False, other_text='其他'):
        ...
        line_count = 0
        debug_file = open(os.path.join(self.eval_dir, "probs.txt"), "w", encoding=cDataset.CHARSET)
        ...
        recall_list = []
        fscore_list = []
        accuracy_dict, precision_dict, recall_dict, fscore_dict = \
        ...
        self.confusion_matrix_list, self. accuracy_list, self.precision_list, self.recall_list,\
            self.fscore_list, self.right_list, self.predict_list, self.standard_list = (
                confusion_matrix_list, accuracy_list,  precision_list, recall_list, fscore_list,
                right_category_count_list, predict_category_count_list,
                standard_category_count_list)
        return (confusion_matrix_list, accuracy_list, precision_list, recall_list, fscore_list,
                right_category_count_list, predict_category_count_list,
                standard_category_count_list)
    ...
    def save(self, eval_name="all"):
        ...
        for i, confusion_matrix in enumerate(self.confusion_matrix_list):
            if i == 0:
                ...
                eval_name = eval_name + "_level_%s" % i
